...
const setup = (dotenvpath = process.env['DOTENV_PATH']) => {
  ...
  const WAIT_FOR_MESSAGE_TIME = 205000;
  const WAIT_FOR_ANOTHER_MESSAGE_TIME = 205000;
    ...
    return new Promise(resolve => {
      ...
      const validateButtons = buttonPayload => {
        const buttonSchema = Joi.object().keys({
          ...
          noinput: Joi.string().allow('').optional(),
          flavor: Joi.string().allow('').optional(), //Frank added this 
          label: Joi.string().allow('').optional(), //Frank added this 
      ...
      try {
        ...
        const tick = async function tick(message) {
          ...
          if (message) {
            ...
            } else if (message.attachment && message.attachment.type && message.attachment.type == 'myaAuth') {
              result.push({ login: message.attachment.label });
            } else if (message.attachment && message.attachment.type && message.attachment.type == 'billingCard' ) {
              // log.error('billingCard info is error:', JSON.stringify(message))
              result.push({ billingCard: true})
              // result.push({ billingCard: 'billing card'})
            } else if (message.attachment && message.attachment.type && message.attachment.type == 'miniSurvey' ) {
              // log.error('miniSurvey info is error:', JSON.stringify(message))
              result.push({ miniSurvey: 'miniSurvey'})
